  ...
  return (
    <div className={`w-1/2 ${type === 'boarding' ? 'pr-2' : 'pl-2'} relative`}>
      <button
        ...
        className={`w-full flex items-center justify-between text-[10px] ${theme === 'dark' ? 'bg-gray-800 hover:bg-gray-700' : 'bg-gray-50 hover:bg-gray-100'} p-1 rounded transition-colors`}
      >
          ...
          {selected ? (
            <>
              ...
              <span className="text-[9px] text-gray-500 dark:text-gray-400 ml-2 flex-shrink-0">
                {options.find(opt => {
                  const name = type === 'boarding' ? opt.boarding_point.name : opt.dropping_point.name;
                  return name === selected;
                }) && convertToIST(options.find(opt => {
                  const name = type === 'boarding' ? opt.boarding_point.name : opt.dropping_point.name;
                  return name === selected;
                })!.currentTime).time}
              </span>
      ...
      {dropdownOpen && (
        ...
        >
          {options.map((point: any) => {
            const name = type === 'boarding' ? point.boarding_point.name : point.dropping_point.name;
            const landmark = type === 'boarding' ? point.boarding_point.landmark : point.dropping_point.landmark;
            const { time } = convertToIST(point.currentTime);
            const isSelected = selected === name;
            
            return (
              <button
                key={type === 'boarding' ? point.boarding_point_id : point.dropping_point_id}
