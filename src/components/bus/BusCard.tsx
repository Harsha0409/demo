...
const BusCard: React.FC<BusCardProps> = ({
  ...
  const handleCardClick = () => {
    ...
    if (bus.recommended_boarding_points?.length) {
      ...
      if (match) displayedBoarding = match.boardingPoint;
    }
    if (!displayedBoarding) {
      displayedBoarding = bus.allBoardingPoints[0]?.boardingPoint || null;
    }
    ...
    if (bus.recommended_dropping_points?.length) {
      ...
      if (match) displayedDropping = match.droppingPoint;
    }
    if (!displayedDropping) {
      displayedDropping = bus.allDroppingPoints[0]?.droppingPoint || null;
    }
    ...
    const eventPayload = {
      ...
      displayed_boarding_point: displayedBoarding
        ? { name: displayedBoarding.name, landmark: displayedBoarding.landmark }
        : null,
      displayed_dropping_point: displayedDropping
        ? { name: displayedDropping.name, landmark: displayedDropping.landmark }
        : null,
    };

    const recommendedSeats = allSeats.map(
      (seat) => `${seat.seat_number}${seat.type === "window" ? "W" : "A"}`
    );
          ...
          <div className="w-1/3 flex flex-col items-start justify-start">
            <div className="text-left mb-2">
              <p className="text-sm font-bold">
                {convertToIST(bus.startTime).time}
              </p>
            ...
            <div className="text-xs">
              ...
              <div className="mt-1 grid grid-cols-3 gap-1">
                {allSeats?.map((seat, index) => (
                  <div
                    key={index}
                    className={`text-[10px] sm:text-xs rounded-md px-1 py-0.5 text-gray-900 flex items-center justify-center ${getSeatBackgroundColor(
                      bus,
                      seat
                    )}`}
                  >
                    {`${seat.seat_number}(${
                      seat.type === "window" ? "W" : "A"
                    })`}
                  </div>
                ))}
              </div>
          ...
          <div className="w-1/3 flex flex-col items-center justify-center">
            <div className="flex flex-col items-center mb-2 w-full">
              ...
              <div className="flex justify-center mt-1">
                <p className="text-sm font-semibold text-[#fbe822]">
                  {bus.duration}
                </p>
              </div>
            </div>
              ...
              {bus.recommended_dropping_points && (
                <div className="text-xs">
                  {bus.recommended_dropping_points.map((point, idx) => (
                    <span key={idx}>
                      {point.name}{" "}
                      {idx < (bus.recommended_dropping_points?.length ?? 0) - 1
                        ? ", "
                        : ""}
                    </span>
                  ))}
            ...
            <div className="text-sm">
              ...
              <div>
                <span className="font-bold">
                  ₹{originalFare.baseFare.toFixed(0)}{" "}
                </span>
