...
const CancellationCard: React.FC<CancellationCardProps> = ({
  ...
  const [selectedTravel, setSelectedTravel] = useState<UpcomingTravel | null>(
    null
  );
  ...
  const handleTravelSelect = (travel: UpcomingTravel) => {
    ...
    const activeSeats = travel.policy.cancelSeatResponseDto.filter(
      (seat) => seat.active
    ).length;
    if (activeSeats === 1) {
      const singleSeat = travel.policy.cancelSeatResponseDto.find(
        (seat) => seat.active
      );
      if (singleSeat) {
  ...
  const toggleSeatSelection = (seatNumber: string) => {
    ...
    if (newSelected.has(seatNumber)) {
      ...
      if (selectedTravel)
        trackGTMEvent("cancel_card_seat_selected", {
      ...
      cashRefund: {
        ...
        percentage:
          selectedSeats.length > 0
            ? selectedSeats[0].seatPolicies.find((p) => !p.isCoinsPolicy)
                ?.percentage || 0
            : 0,
      },
      coinsRefund: {
  ...
  const totalRefundCalculation = useMemo(() => {
    if (!selectedTravel) return null;
    const selectedCount = selectedTravel.policy.cancelSeatResponseDto.length;
    if (selectedCount === 0) {
      return {
        cashRefund: { amount: 0, percentage: 0 },
        coinsRefund: { coins: 0, percentage: 0 },
    ...
    const selectedSeats = selectedTravel.policy.cancelSeatResponseDto;
    let totalCashRefund = 0;
    let totalCoinsRefund = 0;
    selectedSeats.forEach((seat) => {
      const cashPolicy = seat.seatPolicies.find((p) => !p.isCoinsPolicy);
  ...
  const getTotalRefundValue = (policy: any, refundItem: any) => {
    const selectedCount = selectedTravel?.policy.cancelSeatResponseDto.length;
    if (selectedCount === 0) {
  ...
  const getContextualRefundValue = (policy: any, refundItem: any) => {
    ...
    const percentage = percentageMatch ? percentageMatch[1] : "0";
    const selectedSeats =
      selectedTravel?.policy.cancelSeatResponseDto.filter((seat) =>
  ...
  const handleConfirmPayment = async () => {
    ...
    if (uniquePolicyIds.length !== 1) {
      ...
      setTimeout(() => {
        toast.error(
          "Selected seats have inconsistent cancellation policies. Please select seats with the same policy."
        );
      }, 100);
      return;
    }
    ...
    try {
      ...
      const response = await authService.fetchWithRefresh(
        `/api/tickets/${selectedTravel.travel_details.id}/cancel`,
        {
          method: "POST",
          headers: {
        ...
        setTimeout(() => {
          ...
          if (
            ...
            (responseData.statusCode === 400 &&
              errorMessage.includes("selected seat has already been cancelled"))
          ) {
      ...
      if (selectedRefundMethod === "coins") {
        ...
        const cashAmount =
          ...
          dynamicRefundCalculation?.cashRefund.amount ||
          0;
        refundAmountText = `₹${cashAmount.toFixed(2)}`;
      }
      const journeyDate = selectedTravel
        ? formatDateForMessage(selectedTravel.travel_details.date)
        : "N/A";
      const sourceName = selectedTravel?.travel_details.source.name || "N/A";
      const destinationName =
        selectedTravel?.travel_details.destination.name || "N/A";
      const pnr = selectedTravel
        ? `FRE${selectedTravel.travel_details.id}`
        : "N/A";
  ...
  const handleShowRefundPolicies = (show: boolean) => {
    ...
    if (show && !refundPolicyViewed && selectedTravel) {
      ...
      trackGTMEvent("cancel_card_refund_policy_viewed", {
        bookingId: selectedTravel.travel_details.id,
      });
    }
  };
  ...
  return (
    <div
      className={`w-full max-w-7xl mx-auto  ${theme === "dark" ? "dark" : ""}`}
    >
      {data.data.upcoming_travels.length === 0 ? (
        <>
          <div className="p-4 border border-yellow-300 rounded-lg bg-yellow-50 dark:bg-yellow-900/20 dark:border-yellow-700/50">
            <p className="text-center text-yellow-800 dark:text-yellow-200">
              No upcoming travels found to cancel. Please check back later or
              book a new ticket.
            </p>
          </div>
        </>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
          {data.data.upcoming_travels.map((travel) => {
            ...
            const totalPaid = activeSeats.reduce(
              (sum, seat) => sum + (seat.totalFare || 0),
              0
            );
            return (
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to bottom right, #0078d4, #005a9e)",
                }}
                ...
                <div className="flex justify-between items-stretch w-full mt-2 gap-4">
                  <div className="w-1/3 flex flex-col items-start justify-start">
                    <div className="text-left mb-2 w-full">
                      ...
                      <p className="text-sm font-bold truncate">
                        {travel.travel_details.source.name}
                      </p>
                      <p className="text-sm font-bold truncate">
                        {travel.travel_details.source.point}
                      </p>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-1">
                        {travel.policy.cancelSeatResponseDto.map((seat) => {
                          ...
                          return (
                            <span
                              key={seat.seatNumber}
                              className={`rounded-md text-sm font-bold px-2 py-1 inline-flex items-center ${
                                seat.gender.toLowerCase() === "female"
                  ...
                  <div className="w-1/3 flex flex-col items-center justify-center">
                    <div className="flex flex-col items-center mb-2 w-full">
                      <div className="flex items-center w-full">
                        ...
                        <span className="mx-2 flex items-center justify-center">
                          <svg
                            ...
                            className="h-6 w-6 sm:h-5 sm:w-5 text-[#fbe822]"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke="currentColor"
                          >
                      ...
                      <div className="flex justify-center mt-1">
                        <p className="text-base sm:text-sm font-semibold text-[#fbe822]">
                          {duration}
                        </p>
                      </div>
                    </div>
                    <div className="flex p-1 rounded-lg mt-2 bg-[#fbe822]">
                      <span className="text-xs text-[#0078d4] font-bold whitespace-nowrap">
                        Ticket ID: FRE{travel.travel_details.id}
                      </span>
                    </div>
                  </div>
                  <div className="w-1/3 flex flex-col">
                    <div className="text-right mb-2 w-full">
                      ...
                      <p className="text-sm font-bold truncate">
                        {travel.travel_details.destination.name}
                      </p>
                      <p className="text-sm font-bold truncate">
                        {travel.travel_details.destination.point}
                      </p>
                    </div>
      ...
      <CancellationModal
        ...
        totalRefundCalculation={totalRefundCalculation}
        getTotalRefundValue={getTotalRefundValue}
